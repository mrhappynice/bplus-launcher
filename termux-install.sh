#!/usr/bin/env bash
# Installs bplus-launcher into a local 'bplus-launcher' directory,
# downloads the release binary + apps.json, and chmods the binary.

set -Eeuo pipefail

BINARY_URL="https://github.com/mrhappynice/bplus-launcher/releases/download/v0.2.5/bplus-launcher-termux"
JSON_URL="https://github.com/mrhappynice/bplus-launcher/raw/refs/heads/main/apps.json"
INSTALL_DIR="bplus-launcher"
BINARY_NAME="bplus-launcher"
JSON_NAME="apps.json"

# --- helpers ---
have() { command -v "$1" >/dev/null 2>&1; }

download() {
  # $1 = url, $2 = output file
  if have curl; then
    curl -fL --proto '=https' --tlsv1.2 --retry 3 --retry-delay 1 --progress-bar -o "$2" "$1"
  elif have wget; then
    wget --https-only --tries=3 -O "$2" "$1"
  else
    echo "Error: need 'curl' or 'wget' to download files." >&2
    exit 1
  fi
}

# --- work ---
echo "Creating '${INSTALL_DIR}' (if needed)…"
mkdir -p "${INSTALL_DIR}"
cd "${INSTALL_DIR}"

echo "Downloading binary -> ${BINARY_NAME}"
download "${BINARY_URL}" "${BINARY_NAME}"

echo "Downloading json -> ${JSON_NAME}"
download "${JSON_URL}" "${JSON_NAME}"

echo "Making '${BINARY_NAME}' executable…"
chmod +x "${BINARY_NAME}"

echo "Getting UI files.."
mkdir static
cd static
download index.html https://github.com/mrhappynice/bplus-launcher/raw/refs/heads/main/static/index.html
download script.js https://github.com/mrhappynice/bplus-launcher/raw/refs/heads/main/static/script.js
download style.css https://github.com/mrhappynice/bplus-launcher/raw/refs/heads/main/static/style.css

echo "Done ✅"
echo
echo "Files installed to: $(pwd)"
echo " - ${BINARY_NAME}"
echo " - ${JSON_NAME}"
echo
echo "Run it with:"
echo "  ./$(printf %q "${BINARY_NAME}")"
